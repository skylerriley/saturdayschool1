...
// ── Splash: "pulsing gold S" ─────────────────────────────────
// Gold hex-packed balls fill an embossed dark "S" on a diagonal sweep while
// holographic tracers race around the un-filled outline. Adapted from the
// s-pulse standalone; the ball grid that was injected by a <script> there is
// generated at module load into SPLASH_BALLS and rendered as React elements.
// --ss-dur controls the whole cycle length.
const SPLASH_CSS = `
  .ss-splash{
    position:fixed;inset:0;display:flex;align-items:center;justify-content:center;
    background:#0B2E1A url('/oakcreek-3.jpg') center/cover no-repeat;overflow:hidden;
  }
  /* dark green scrim over the hole image so the gold S reads on top */
  .ss-splash::before{
    content:'';position:absolute;inset:0;
    background:rgba(11,46,26,0.40);
    pointer-events:none;
  }
  :root{ --ss-dur:3s; }
  .ss-splash__mark{ position:relative;z-index:1;overflow:visible;width:min(149px,20.7vw);height:auto;max-height:23.4vh; }

  /* diagonal (top-left -> bottom-right) sweep used to clip the tracers to the un-filled area */
  @keyframes ss-wipeOut {
  ...
  /* each ball pops in, holds, pops out */
  @keyframes ss-ballPop {
    0%   { transform: scale(0); }
  ...
  @keyframes ss-racerMoveA { to { stroke-dashoffset: -300; } }
  @keyframes ss-racerMoveB { to { stroke-dashoffset: -318; } }
  @keyframes ss-baseOutline {
    0%,44% { opacity:.35; }
    58%    { opacity:0; }
    100%   { opacity:0; }
  }
  .ss-splash__sweep   { transform-box: view-box; transform-origin: 150px 180px; animation: ss-wipeOut var(--ss-dur) linear infinite; }
  .ss-splash__ball    { transform-box: fill-box; transform-origin: center; animation: ss-ballPop var(--ss-dur) cubic-bezier(.34,1.4,.5,1) infinite backwards; }
  .ss-splash__outline { animation: ss-baseOutline var(--ss-dur) ease-in-out infinite; }
  .ss-splash__racerA  { animation: ss-racerMoveA calc(var(--ss-dur) * 0.16) linear infinite; }
  .ss-splash__racerB  { animation: ss-racerMoveB calc(var(--ss-dur) * 0.13) linear infinite; }

  /* Reduced motion: show a static filled gold S, no sweep/tracers/pops.
     The splash timeout is also shortened in code (see splashDone effect). */
  @media (prefers-reduced-motion:reduce){
    .ss-splash__ball{ animation:none!important;transform:none; }
    .ss-splash__outline,.ss-splash__racerA,.ss-splash__racerB{ display:none!important; }
  }
`;

// Hex-packed gold ball grid, generated once at module load (was the <script>
// in the standalone file). Delay is proportional to each ball's position along
// the top-left -> bottom-right diagonal so the fill sweeps in.
const SPLASH_BALLS = (() => {
  ...
  // Deterministic per-ball jitter (no Math.random — keeps builds/SSR stable).
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* Like/comment bar on auto beats. flex:0 0 auto so it keeps its own row under
     the "Auto highlight" line; pointer-events:auto because the .beat ancestor is
     pointer-events:none (same reason .hl-seen-chip needs it). */
  .beat-actions{flex:0 0 auto;pointer-events:auto;padding-top:12px;}
  ...
  if(loading || !splashDone) return(
    <>
      ...
      <div className="ss-splash" data-play="true" onClick={()=>setSplashDone(true)} style={{cursor:"pointer"}}>
        <svg className="ss-splash__mark" viewBox="0 0 300 360">
          <defs>
            <linearGradient id="ssHoloGrad" x1="0" y1="0" x2="1" y2="1">
            ...
            <clipPath id="ssDarkClip">
              <rect className="ss-splash__sweep" x="-500" y="0" width="1300" height="1000"></rect>
            </clipPath>
            <filter id="ssEmboss" x="-20%" y="-20%" width="140%" height="140%">
          ...
          <text x="150" y="180" textAnchor="middle" dominantBaseline="central" fontFamily="'DM Sans',sans-serif" fontWeight={900} fontSize="300" fill="#2a2a2f" filter="url(#ssEmboss)">S</text>

          {/* faint outline while empty */}
          <text className="ss-splash__outline" x="150" y="180" textAnchor="middle" dominantBaseline="central" fontFamily="'DM Sans',sans-serif" fontWeight={900} fontSize="300" fill="none" stroke="#5a5a62" strokeWidth="1.5">S</text>

          {/* gold fill: hex-packed balls */}
          <g clipPath="url(#ssSClip)">
            {SPLASH_BALLS.map((b,i)=>(
              <circle key={i} className="ss-splash__ball" cx={b.cx} cy={b.cy} r={b.r} fill="#ffab00"
                style={{animationDelay:`calc(var(--ss-dur) * ${b.delay})`}}/>
            ))}
          </g>

          {/* holographic tracers, clipped to the un-filled portion */}
          <text className="ss-splash__racerA" x="150" y="180" textAnchor="middle" dominantBaseline="central" fontFamily="'DM Sans',sans-serif" fontWeight={900} fontSize="300" fill="none" stroke="url(#ssHoloGrad)" strokeWidth="2" strokeLinecap="round" strokeDasharray="190 110" clipPath="url(#ssDarkClip)">S</text>
          <text className="ss-splash__racerB" x="150" y="180" textAnchor="middle" dominantBaseline="central" fontFamily="'DM Sans',sans-serif" fontWeight={900} fontSize="300" fill="none" stroke="#e8dcc0" strokeWidth="2" strokeLinecap="round" strokeDasharray="26 292" clipPath="url(#ssDarkClip)">S</text>
